...
MATCH_RATE = 24000       # 匹配采样率(降采样以加快匹配)
CHUNK_SEC = 0.1          # 每次采集时长(秒)
BUFFER_SEC = 6.0         # 匹配缓冲区时长(秒), 需大于比对音频长度
THRESHOLD = 0.4          # 匹配阈值 0~1: 越大越严格(误触发少, 但可能漏检)
...
MIN_RATE = 0.3   # 最低倍率(固定)
MAX_RATE = 4.0   # 最高倍率(固定)
        ...
        try:
            while not self._stop.is_set():
                ...
                for name, tmpl in self.templates.items():
                    ...
                    if c > THRESHOLD and (best is None or c > best[1]):
                        best = (name, c)
                if best:
                    self.evq.put(("trigger", best[0], best[1]))
                    buf = np.zeros(0, dtype=np.float32)
...
class PlaybackManager:
    ...
    def __init__(self, evq):
        ...
        self._speaker = None
        self._nch = 2
        self.aowu_gain = 1.0   # aowu.wav 播放音量(0~1, 界面可调)
        threading.Thread(target=self._worker, daemon=True).start()

    def play(self, rate):
        """请求播放 flash + aowu(倍率 rate); 若有正在播放的音效则打断"""
        self.gen += 1
        try:
            self.cmd.get_nowait()
        except queue.Empty:
            pass
        try:
            self.cmd.put_nowait((rate, self.gen))
        except queue.Full:
            pass
    ...
    def _worker(self):
        self._preload()
        while True:
            rate, gen = self.cmd.get()
            try:
                self._play_sequence(rate, gen)
            except Exception as e:
    ...
    def _preload(self):
        ...
        for path in (FLASH_WAV, AOWU_WAV):
            if not os.path.exists(path):
    ...
    def _play_sequence(self, rate, gen):
        if self._interrupted(gen):
            return
        for path, r, gain in ((FLASH_WAV, 1.0, FLASH_GAIN), (AOWU_WAV, rate, self.aowu_gain)):
            if self._interrupted(gen):
                break
...
class ImageFlash:
    ...
    def show(self, black_bg=False):
        try:
            from PIL import Image, ImageTk
        except ImportError:
            self.evq.put(("error", "缺少 Pillow, 图片功能不可用"))
            return
        path = AOWU_IMG if os.path.exists(AOWU_IMG) else AOWU_IMG_JPG
        if not os.path.exists(path):
            self.evq.put(("error", f"找不到图片: {AOWU_IMG}"))
            return
        try:
            sw = self.root.winfo_screenwidth()
            sh = self.root.winfo_screenheight()
...
class App:
    ...
    def _build_ui(self):
        ...
        row += 1
        self.count_lbl = ttk.Label(frm, text="统计次数: 0 次", font=("", 11, "bold"))
        self.count_lbl.grid(row=row, column=0, sticky="w")
        self.rate_lbl = ttk.Label(frm, text=f"当前倍率: x{self.min_rate:.2f}", font=("", 11, "bold"))
        self.rate_lbl.grid(row=row, column=1, sticky="w", padx=(16, 0))

        row += 1
        vol = ttk.LabelFrame(frm, text="播放音量", padding=8)
        vol.grid(row=row, column=0, columnspan=2, sticky="ew", pady=(4, 4))
        ...
        row += 1
        cond = ttk.LabelFrame(frm, text="播放条件", padding=8)
        cond.grid(row=row, column=0, columnspan=2, sticky="ew", pady=(8, 4))
        ttk.Checkbutton(cond, text="捕捉成功", variable=self.success_var).grid(row=0, column=0, sticky="w")
        ttk.Checkbutton(cond, text="捕捉失败", variable=self.fail_var).grid(row=1, column=0, sticky="w")
        ttk.Checkbutton(cond, text="球命中", variable=self.hit_var).grid(row=2, column=0, sticky="w")
        ...
        ttk.Button(test, text="测试(图片+音频)", width=16, command=self._on_test).grid(row=0, column=0, sticky="w")
        ttk.Button(test, text="+1", width=6, command=self._on_plus_one).grid(row=0, column=1, sticky="w", padx=(8, 0))
        ttk.Button(test, text="打断", width=6, command=self._on_interrupt).grid(row=0, column=2, sticky="w", padx=(8, 0))
    ...
    def _update_count_label(self):
        self.count_lbl.config(text=f"统计次数: {self.count} 次")
    ...
    def _increment_count(self):
        """统计 +1(永不自动清零); 倍率计数 +1, 超过上限时倍率计数归零; 返回是否重置了倍率"""
        self.count += 1
        self.rate_count += 1
    ...
    def _on_trigger(self, name, corr):
        ...
        self._log(f"检测到「{label}」音效 (相关度 {corr:.2f}) → 统计 {self.count}, 播放倍率 x{rate:.2f}")
        self._play_and_flash(rate)
    ...
    def _play_and_flash(self, rate):
        self._update_count_label()
        self.player.play(rate)
        if self.image_var.get():
            self.flash.show(self.black_bg_var.get())

    def _on_test(self):
        rate = self._current_rate()
        self._log(f"测试: 播放 flash.wav → aowu.wav (倍速 x{rate:.2f}) 并全屏显示图片")
        self.player.play(rate)
        self.flash.show(self.black_bg_var.get())
